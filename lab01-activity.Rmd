...
pwd
```
## /local-scratch/localhome/chloep/lab-1-Chloeep
...
Answer: Creates a folder called "genotype_head.txt". Also '>>" appends the data to the end of the contents of the file while '>' wipes out the initial data and writes over

* What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? Why do you think that is?
Answer: Error - No directory or file called "out"
...
Answer: "--" is not a genotype, but is included. To solve this, pipe the output to command 'grep -v "\-\-"'

Cut -f 4 = returns the data in the fourth column
Sort = Sorts the data into alphabetical order
Uniq = returns data frame without duplicates
...
Answer: Each time I run the file, the phrase "Hello terminal!" and the date is added to the pre-existing content of the same file
...
Make a copy of the code chunk above and put it below this text in your version of the Rmd file. Modify the code so it instead creates a file with this exact name:

`~/.bash_profile`. *General hint*: when you can, use copy/paste so you don't have to re-type a command or file name. You also need to change the code chunk to add a single line to the new file. The first line of text should be:
...
echo source ~/.bashrc $'\n'DATA=/local-scratch/course_files/MBB243/ > ~/.bash_profile
```
...
```{bash}
echo $(samtools faidx /local-scratch/course_files/MBB243/hg38.fa chr14:65102304-65102339 | grep -v \> > optional_task.txt)
```

Think about how you might make this code output to a file instead of just to the terminal. How might this be useful? 

By making this code output a set of nucleotides into a file, I can easily retrieve this set of nucelotide by assigning a variable to the file context. so in the future, I don't have to write out the whole code to refer to the sequence, instead, I can just use the variable that was assigned. 
